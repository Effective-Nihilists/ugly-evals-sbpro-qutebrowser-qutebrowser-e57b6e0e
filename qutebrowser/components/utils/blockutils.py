...
class BlocklistDownloads(QObject):
    ...
    Signals:
        single_download_finished: Emitted when a single download has finished.
            Provides the file object (IO[bytes]).
        all_downloads_finished: Emitted when all downloads have finished.
            Provides the number of items downloaded (int).
    ...
    Attributes:
        ...
        _finished_registering_downloads:
            ...
            queue, we don't call all_downloads_finished prematurely.
        _started: Has the `initiate` method been called?
        _finished: Have all downloads finished?
    """
    ...
    def __init__(
        ...
        on_single_download: typing.Optional[
            typing.Callable[[typing.IO[bytes]], typing.Any]
        ] = None,
        ...
        self._urls = urls
        if on_single_download is not None:
            self.single_download_finished.connect(on_single_download)
        if on_all_downloaded is not None:
            self.all_downloads_finished.connect(on_all_downloaded)
    ...
    def _on_download_finished(self, download: downloads.TempDownload) -> None:
        """Check if all downloads are finished and if so, emit signals.
