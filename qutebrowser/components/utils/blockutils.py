...
class BlocklistDownloads(QObject):
    ...
    Signals:
        single_download_finished:
            Emitted when a single download has finished. The download's
            file-like object is passed as the argument.
        all_downloads_finished:
            Emitted when all downloads have finished. The number of
            successfully completed downloads is passed as the argument.
    """
    ...
    def _on_download_finished(self, download: downloads.TempDownload) -> None:
        """Check if all downloads are finished and if so, emit the signal.
